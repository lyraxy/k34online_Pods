...
class KeychainHelper {
    ...
    private let service = "com.k34.online"
...
class KeyManager: ObservableObject {
    ...
    private let keyTag = "com.k34.online.encryptionKeys"
...
class CryptoService: ObservableObject {
    ...
    private var roomSecrets: [String: Data] = [:]
    
    private init() {
        self.encryptionKeys = keyManager.getOrCreateKeyPair()
    }
    ...
    func getRoomSecret(roomId: String) -> Data? {
    ...
    func sendKeyExchange(to roomId: String, matrixService: MatrixService) {
        let publicKeyBase64 = getMyPublicKey()
    ...
    func encryptMessage(_ text: String, roomId: String) -> String? {
...
struct ContentView: View {
    ...
    var mainView: some View {
        ZStack {
            ...
            TabView(selection: $selectedTab) {
                ...
                NewChatView(matrixService: matrixService, cryptoService: cryptoService)
                    .tabItem {
                ...
                ProfileView(matrixService: matrixService, cryptoService: cryptoService)
                    .tabItem {
    ...
    var loginView: some View {
        VStack(spacing: 30) {
            ...
            Text("v1.0 secure-build • End-to-End шифрование")
                .font(.caption)
...
struct ChatListView: View {
    ...
    private func chatRow(for room: MXRoom) -> some View {
        ZStack {
            ...
            ChatRow(room: room, matrixService: matrixService, cryptoService: cryptoService, onLeaveRoom: {
                roomToLeave = room
...
struct ChatRow: View {
    ...
    var body: some View {
            ...
            ZStack(alignment: .bottomTrailing) {
                ZStack {
                    ...
                    if roomStatus?.isEncrypted == true {
                        Circle()
                            .stroke(K34Colors.encryptedGreen, lineWidth: 2)
                            .frame(width: 50, height: 50)
                    }
                }
        ...
        .contextMenu {
            ...
            if roomStatus?.isEncrypted == true {
                Button {
                    cryptoService.sendKeyExchange(to: room.roomId, matrixService: matrixService)
                } label: {
                    Label("Повторить обмен ключами", systemImage: "key.fill")
                }
            }
        }
    }
            ...
            NavigationView {
                VStack(spacing: 30) {
                    ...
                    VStack(spacing: 20) {
                        ...
                        Toggle("Включить End-to-End шифрование", isOn: $enableEncryption)
                            .padding()
                            .background(K34Colors.cardBackground)
                            .cornerRadius(12)
                            .padding(.horizontal)
                        ...
                        Button(action: createDirectChat) {
                            if isCreatingRoom {
                                ...
                                HStack {
                                    Image(systemName: enableEncryption ? "lock.fill" : "message.fill")
                                    Text(enableEncryption ? "Создать безопасный чат" : "Начать чат")
                                        .fontWeight(.semibold)
                                }
...
struct ChatRoomView: View {
    ...
    var body: some View {
                ...
                Menu {
                    ...
                    if roomStatus?.isEncrypted == true {
                        Button {
                            ...
                            Label("Обмен ключами", systemImage: "key.fill")
                        }
                    }
                } label: {
        ...
        .sheet(isPresented: $showingVoiceRecorder) {
            VoiceMessageRecorderView(
                ...
                onSend: { audioData in
                    matrixService.sendVoiceMessage(audioData, in: room.roomId, cryptoService: cryptoService)
                }
            )
        }
        .fileImporter(
            ...
            if roomStatus?.isEncrypted == true {
                HStack {
                    Image(systemName: "lock.fill")
                        .font(.system(size: 12))
                        .foregroundColor(K34Colors.encryptedGreen)
                    Text("Чат защищен End-to-End шифрованием")
                        .font(.caption)
                        .foregroundColor(K34Colors.encryptedGreen)
                }
                .padding(8)
                .background(K34Colors.encryptedGreen.opacity(0.1))
                .cornerRadius(8)
                .padding(.horizontal)
                .padding(.top, 8)
            }
    ...
    private func sendMessage() {
        ...
        matrixService.sendMessage(trimmedText, in: room.roomId, cryptoService: cryptoService)
        messageText = ""
    }
    ...
    private func handleFileSelection(_ result: Result<[URL], Error>) {
        ...
        case .success(let urls):
            ...
            matrixService.sendFile(url, in: room.roomId, cryptoService: cryptoService)
...
struct MessageBubble: View {
        ...
        VStack(alignment: message.isOutgoing ? .trailing : .leading, spacing: 4) {
            HStack {
                    ...
                    if message.messageType == .text {
                        HStack(alignment: .top, spacing: 4) {
                            if message.isEncrypted {
                                Image(systemName: getEncryptionIcon())
                                    .font(.system(size: 10))
                                    .foregroundColor(getEncryptionColor())
                            }
                ...
                .contextMenu {
                    ...
                    if message.messageType == .text {
                        Button {
                            UIPasteboard.general.string = getMessageText()
                        } label: {
    ...
    private func getEncryptionIcon() -> String {
        switch message.encryptionStatus {
        case .encrypted:
            return "lock.fill"
        case .decrypted:
    ...
    private func getEncryptionColor() -> Color {
        switch message.encryptionStatus {
        case .encrypted:
            return K34Colors.lightGray
        case .decrypted:
        ...
        case .failed:
            return K34Colors.lightRed
        case .pending:
            return K34Colors.textSecondary
        }
    }
...
// MARK: - Key Exchange Message View
struct KeyExchangeMessageView: View {
    var body: some View {
        HStack(spacing: 8) {
        ...
        .overlay(
            RoundedRectangle(cornerRadius: 12)
                .stroke(K34Colors.encryptedGreen, lineWidth: 1)
        )
    }
}
            ...
            NavigationView {
                VStack(spacing: 30) {
                    ...
                    VStack(spacing: 15) {
                        ...
                        Text("Все ваши сообщения защищены современным шифрованием. Ключи хранятся только на вашем устройстве.")
                            .font(.caption)
                            .foregroundColor(K34Colors.textSecondary)
                    ...
                    VStack(spacing: 10) {
                        Text("K-34 Online Secure")
                            .font(.caption)
                            .foregroundColor(K34Colors.textSecondary)
                        
                        Text("Безопасно • Зашифровано • Надежно")
                            .font(.caption2)
                            .foregroundColor(K34Colors.lightGray)
                    }
                    .padding(.bottom, 20)
